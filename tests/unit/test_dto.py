...
class TestRegisterRequestDTO:
    ...
    def test_valid_registration_minimal(self):
        ...
        dto = RegisterRequestDTO(
            email="test@example.com", password="SecurePass123!", first_name="Test"
        )
    ...
    def test_invalid_email(self):
        ...
        with pytest.raises(ValidationError) as exc:
            RegisterRequestDTO(
                email="invalid-email", password="SecurePass123!", first_name="Test"
            )
    ...
    def test_weak_password(self):
        ...
        with pytest.raises(ValidationError) as exc:
            RegisterRequestDTO(
                email="test@example.com", password="123", first_name="Test"
            )
    ...
    def test_empty_first_name(self):
        ...
        with pytest.raises(ValidationError) as exc:
            RegisterRequestDTO(
                email="test@example.com", password="SecurePass123!", first_name=""
            )
    ...
    def test_too_long_first_name(self):
        ...
        with pytest.raises(ValidationError) as exc:
            RegisterRequestDTO(
                email="test@example.com", password="SecurePass123!", first_name="A" * 51
            )
        assert "first_name" in str(exc.value)
...
class TestRegisterResponseDTO:
    ...
    def test_custom_message(self):
        ...
        dto = RegisterResponseDTO(user_id="123", message="Custom message")
...
class TestLoginRequestDTO:
    ...
    def test_login_with_email(self):
        ...
        dto = LoginRequestDTO(login="test@example.com", password="SecurePass123!")
        assert dto.login == "test@example.com"
        assert dto.password == "SecurePass123!"

    def test_login_with_username(self):
        """Тест логина по username"""
        dto = LoginRequestDTO(login="testuser", password="SecurePass123!")
        assert dto.login == "testuser"

    def test_login_with_email_alias(self):
        """Тест логина с алиасом email"""
        dto = LoginRequestDTO(email="test@example.com", password="SecurePass123!")
        assert dto.login == "test@example.com"

    def test_login_too_short(self):
        """Тест короткого логина"""
        with pytest.raises(ValidationError) as exc:
            LoginRequestDTO(login="ab", password="SecurePass123!")
        assert "login" in str(exc.value)

    def test_empty_password(self):
        """Тест пустого пароля"""
        with pytest.raises(ValidationError) as exc:
            LoginRequestDTO(login="test@example.com", password="")
        assert "password" in str(exc.value)
...
class TestTokenResponseDTO:
    ...
    def test_minimal_token_response(self):
        ...
        dto = TokenResponseDTO(access_token="access-token-123", expires_in=300)
...
class TestResetPasswordRequestDTO:
    ...
    def test_valid_reset_password(self):
        ...
        dto = ResetPasswordRequestDTO(
            key="valid-reset-token-123", new_password="NewSecurePass123!"
        )
    ...
    def test_empty_key(self):
        """Тест пустого ключа"""
        with pytest.raises(ValidationError) as exc:
            ResetPasswordRequestDTO(key="", new_password="SecurePass123!")
        assert "key" in str(exc.value)

    def test_weak_new_password(self):
        """Тест слабого нового пароля"""
        with pytest.raises(ValidationError) as exc:
            ResetPasswordRequestDTO(key="valid-token", new_password="123")
        assert "new_password" in str(exc.value)

    def test_empty_new_password(self):
        """Тест пустого нового пароля"""
        with pytest.raises(ValidationError) as exc:
            ResetPasswordRequestDTO(key="valid-token", new_password="")
        assert "new_password" in str(exc.value)
