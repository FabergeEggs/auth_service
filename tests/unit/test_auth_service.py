...
class TestAuthServiceRegistration:
    ...
    async def test_register_user_already_exists(
        self, mock_auth_provider, test_user_data
    ):
    ...
    @pytest.mark.asyncio
    async def test_register_re_raises_user_exists_after_conflict(
        self, mock_auth_provider, test_user_data
    ):
        """Тест конфликта, после которого пользователь уже найден."""
        mock_auth_provider.get_user_by_username.side_effect = [
            None,
            {"id": "existing-id"},
        ]
    ...
    @pytest.mark.asyncio
    async def test_register_sends_event_when_producer_present(
        self, mock_auth_provider, test_user_data
    ):
    ...
    @pytest.mark.asyncio
    async def test_register_ignores_event_producer_errors(
        self, mock_auth_provider, test_user_data
    ):
...
class TestAuthServiceLogin:
    ...
    @pytest.mark.asyncio
    async def test_login_extracts_user_id_from_access_token(
        self, mock_auth_provider, test_login_data
    ):
    ...
    @pytest.mark.asyncio
    async def test_login_returns_none_if_token_payload_invalid(
        self, mock_auth_provider, test_login_data
    ):
...
class TestAuthServicePasswordRecovery:
    ...
    @pytest.mark.asyncio
    async def test_forgot_password_does_not_send_for_unknown_user(
        self, mock_auth_provider
    ):
        mock_auth_provider.get_user_by_username.return_value = None
        auth_service = AuthService(mock_auth_provider)
...
class TestAuthServicePasswordChange:
    ...
    async def test_request_password_change_sends_email(self, mock_auth_provider):
        ...
        mock_auth_provider.get_user_by_username = AsyncMock(
        ...
        mock_auth_provider.send_reset_password_email.assert_awaited_once_with(
            "user-123"
        )
